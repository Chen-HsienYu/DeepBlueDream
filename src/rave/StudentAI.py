...
class MCTS():
    ...
    def search(self, time_limit) -> Move:
        ...
        while time() < timeout:
            ...
            while not win_val:
                move = get_random_move(temp_board, temp_color)
                moves_list[temp_color - 1].add(str(move))
            ...
            node.backpropogate(win_for_parent, moves_list)
...
class TreeNode():
    ...
    def backpropogate(self, win_for_parent, moves_list) -> None:
        '''
        ...
        if self.parent:
            self.parent.backpropogate(-win_for_parent, moves_list)
            
            # AMAF
            for move in self.parent.children.keys():
                if str(move) in moves_list[self.color-1]:   
                    if self.parent.children[move] is None:
                        self.parent.children[move] = TreeNode(self.parent.board, self.color, move, self.parent)
                                         
                    if win_for_parent == 1:
                        self.parent.children[move].amaf_wins += 1
                    self.parent.children[move].amaf_visits += 1
                    
                    self.parent.children[move].update_ucb()
    ...
    def update_ucb(self):
        alpha = 1 / (self.visit_count/10) 
        standard = self.wins_for_parent/self.visit_count + C_VAL * sqrt(log(self.parent.visit_count)/self.visit_count)
        self.ucb_value = alpha*self.amaf_wins/self.amaf_visits + (1-alpha)*self.wins_for_parent/self.visit_count + C_VAL*sqrt(log(self.parent.visit_count)/self.visit_count)
